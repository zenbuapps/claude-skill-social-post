# Zenbu Apps 改造版：內容庫 × 行動發文 × 團隊追蹤

> 這個 fork 保留原始 `social-post` skill 的發文公式、語氣學習與社群平台操作邏輯，並加入 Zenbu Apps 的內容營運架構：用 Obsidian 保存深度素材，用 Notion 管理團隊可見的發文流程，讓 Codex/Claude 可以從手機端引導使用者完成選題、媒合、寫作、排程與成效追蹤。

## 我們改了什麼

這個版本把原本「今天發一篇」的單點生成流程，擴充成一套可多人協作的內容工作流：

1. **手機啟動與引導**
   - 使用者可以直接用手機連線 Codex，說「幫我發一篇」或「今天有個事件想寫」。
   - skill 會先詢問是否有主題、生活事件、圖片、影片或特定素材，而不是直接開始寫。

2. **策略判斷**
   - 依照主題判斷適合的發文公式，必要時可混合多個公式。
   - 先決定短文、中篇或長文，再依 Threads、Facebook、IG 等平台特性調整文案。
   - 在產出前確認要發布的平台，以及不同平台是否要使用不同圖文或影片。

3. **Obsidian 內容庫媒合**
   - 深度內容、YouTube 逐字稿、個人生活事件、產品開發紀錄等素材，預設存放在 Obsidian。
   - 使用者給一個主題後，skill 會先到內容庫尋找可搭配的素材。
   - 正式寫作前，至少提出 3 個可發展方向讓使用者選擇。

4. **Notion 團隊流程管理**
   - Notion 不放大量逐字稿，主要保存標題、metadata、發文狀態、排程時間、平台、負責人與成效紀錄。
   - 適合讓團隊共同查看哪些素材已使用、哪些貼文已排程、哪些貼文需要追蹤。

5. **發布後回顧**
   - 發文 24 小時後可回顧數據，將觸及、互動、留言、儲存等結果回填 Notion。
   - 長期累積受眾偏好，讓後續選題與公式選擇更貼近真實受眾反應。

## 建議資料架構

- **Obsidian**：完整逐字稿、內容筆記、生活事件、產品開發紀錄、客戶故事、受眾洞察。
- **Notion**：素材索引、發文排程、平台版本、團隊協作狀態、24 小時數據回填。
- **Codex / Claude**：負責從內容庫找素材、判斷公式、產出文案、引導修稿與操作發文流程。

## Zenbu 使用入口

啟動 Codex 或 Claude 後，可以直接說：
...
1. 問你今天有沒有主題、事件、圖片或影片。
2. 判斷適合的發文公式、文章長度與平台策略。
3. 從 Obsidian 內容庫找出至少 3 個可搭配方向。
4. 等你選定方向後，產出不同平台版本。
5. 修到確認後，再詢問排程時間並記錄到 Notion。

完整改造流程請見 [`social-post/references/zenbu_mobile_workflow.md`](social-post/references/zenbu_mobile_workflow.md)。
