---
name: social-post
description: 本地別名「發文大師」。引導使用者從手機或桌面完成社群發文：先問主題/生活事件/圖片影片，再判斷發文公式、長度與平台；可選「內容庫媒合」或「快速模式：不查內容庫，直接用主題+公式寫」。需要素材時從 Obsidian 內容庫媒合至少 3 個方向；不需要素材時直接產出 FB / IG / Threads 文案，確認後協助排程、記錄到 Notion、追蹤 24 小時數據並累積受眾偏好。使用時機：使用者說「發文大師」、「請發文大師幫我發一篇文」、「發文」、「幫我寫貼文」、「今天發一篇」、「PO 一下」、「用我的風格發」、「找內容庫搭配」、「不要查內容庫」、「快速寫」、「排貼文」、「追蹤數據」、「review」時觸發。
---
...
3. 選路徑：若使用者要短文/即時感想/明確說不要查內容庫，走**快速模式**，直接用「主題 + 候選公式」寫；若需要深度素材或長文，才搜尋 Obsidian 內容庫並提出 **3 個可選方向**。
4. 內容庫模式等使用者選方向後才寫；快速模式可在策略診斷後直接寫各平台草稿。
